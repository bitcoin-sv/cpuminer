...
static bool gmc_work_decode(const json_t *val, struct work *work) {
  ...
  if (!prevhash || !version_val || !nBits_val || !nTime_val || !height_val ||
      !id) {
  ...
  for (int i = 0; i < 8; i++) work->target[7 - i] = 0;
...
static inline void work_copy(struct work *dest, const struct work *src) {
  ...
  if (src->txs) dest->txs = strdup(src->txs);
  if (src->workid) dest->workid = strdup(src->workid);
  if (src->job_id) dest->job_id = strdup(src->job_id);
  if (src->xnonce2) {
...
static bool jobj_binary(const json_t *obj, const char *key, void *buf,
  ...
  if (!hex2bin(buf, hexstr, buflen)) return false;
...
static bool work_decode(const json_t *val, struct work *work) {
  ...
  if (unlikely(
          !jobj_binary(val, "target", work->target, sizeof(work->target)))) {
...
static bool gbt_work_decode(const json_t *val, struct work *work) {
  ...
  if (tmp && json_is_array(tmp)) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (!s) continue;
      if (!strcmp(s, "segwit") || !strcmp(s, "!segwit")) segwit = true;
    }
  }
  ...
  if (tmp) {
    ...
    cbvalue =
        json_is_integer(tmp) ? json_integer_value(tmp) : json_number_value(tmp);
    cbtx = malloc(256);
    ...
    if (work->height >= 1 && work->height <= 16) {
      ...
      for (n = work->height; n; n >>= 8) {
        ...
        if (n < 0x100 && n >= 0x80) cbtx[cbtx_size++] = 0;
      }
    ...
    if (segwit) {
      ...
      n = tx_count + 1;
      while (n > 1) {
        if (n % 2) memcpy(wtree[n], wtree[n - 1], 32);
        n = (n + 1) / 2;
        for (i = 0; i < n; i++) sha256d(wtree[i], wtree[2 * i], 64);
      }
  ...
  if (coinbase_append) {
    ...
    if (xsig_len) {
      ...
      cbtx[41] += n;
      if (push_len == 2) *(ssig_end++) = 0x4c; /* OP_PUSHDATA1 */
      if (push_len) *(ssig_end++) = xsig_len;
  ...
  while (n > 1) {
    ...
    n /= 2;
    for (i = 0; i < n; i++) sha256d(merkle_tree[i], merkle_tree[2 * i], 64);
  }

  /* assemble block header */
  work->data[0] = swab32(version);
  for (i = 0; i < 8; i++) work->data[8 - i] = le32dec(prevhash + i);
  for (i = 0; i < 8; i++)
...
static void share_result(int result, const char *reason) {
  ...
  applog(LOG_INFO, "accepted: %lu/%lu (%.2f%%), %s khash/s %s", accepted_count,
...
static bool submit_mining_solution(CURL *curl, const struct work *work) {
  ...
  if (!rc) {
    ...
    const char *reason =
        err ? json_string_value(json_object_get(err, "message")) : NULL;
...
static bool submit_upstream_work(CURL *curl, struct work *work) {
  ...
  if (!submit_old && memcmp(work->data + 1, g_work.data + 1, 32)) {
    if (opt_debug) applog(LOG_DEBUG, "DEBUG: stale work detected, discarding");
    return true;
  }
  ...
  if (have_stratum) {
    ...
    req = malloc(256 + strlen(rpc_user) + strlen(work->job_id) +
                 2 * work->xnonce2_len);
    sprintf(req,
...
static bool submit_work(struct thr_info *thr, const struct work *work_in) {
  ...
  if (!tq_push(thr_info[work_thr_id].q, wc)) goto err_out;
...
static void *miner_thread(void *userdata) {
  ...
  while (1) {
    ...
    if (have_stratum) {
      ...
      pthread_mutex_lock(&g_work_lock);
      if (!have_stratum && (time(NULL) - g_work_time >= min_scantime ||
    ...
    if (!opt_quiet) {
      ...
      applog(LOG_INFO, "thread %d: %lu hashes, %s khash/s", thr_id, hashes_done,
             s);
    }
...
static void restart_threads(void) {
  ...
  for (i = 0; i < opt_n_threads; i++) work_restart[i].restart = 1;
}
...
start:
  ...
  /* absolute path, on current server */
    ...
    if (rpc_url[strlen(rpc_url) - 1] != '/') need_slash = true;
...
static bool stratum_handle_response(char *buf) {
  ...
  if (!id_val || json_is_null(id_val) || !res_val) goto out;
...
static void *stratum_thread(void *userdata) {
  ...
  while (1) {
    ...
    if (!stratum_handle_method(&stratum, s)) stratum_handle_response(s);
    free(s);
  }
...
int main(int argc, char *argv[]) {
  ...
  if (opt_background) {
    ...
    if (i > 0) exit(0);
    i = setsid();
    if (i < 0) applog(LOG_ERR, "setsid() failed (errno = %d)", errno);
    i = chdir("/");
    if (i < 0) applog(LOG_ERR, "chdir() failed (errno = %d)", errno);
  ...
  if (!opt_n_threads) opt_n_threads = num_processors;
  ...
  if (want_stratum) {
    ...
    if (have_stratum) tq_push(thr_info[stratum_thr_id].q, strdup(rpc_url));
  }
