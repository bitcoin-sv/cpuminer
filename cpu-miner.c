...
static bool gmc_work_decode(const json_t* val, struct work* work)
{
    ...
    if (!tmp || !json_is_integer(tmp)) {
        applog(LOG_ERR, "JSON GMC invalid height");
        goto out;
    }
    ...
    if (!tmp || !json_is_integer(tmp)) {
        applog(LOG_ERR, "JSON GMC invalid version");
        goto out;
    }
    version = json_integer_value(tmp);

    if (unlikely(!jobj_binary(val, "prevhash", prevhash, sizeof(prevhash)))) {
        applog(LOG_ERR, "JSON GMC invalid prevhash");
        goto out;
    }

    tmp = json_object_get(val, "time");
    if (!tmp || !json_is_integer(tmp)) {
        applog(LOG_ERR, "JSON GMC invalid time");
        goto out;
    }
    curtime = json_integer_value(tmp);

    if (unlikely(!jobj_binary(val, "nBits", &bits, sizeof(bits)))) {
        applog(LOG_ERR, "JSON GMC invalid bits");
        goto out;
    }
    ...
    if (!tmp || !json_is_string(tmp)) {
        applog(LOG_ERR, "JSON GMC invalid workid");
        goto out;
    }
    work->workid = strdup(json_string_value(tmp));

    /* convert bits to target */
    uint32_t bitsrev = swab32(bits);
    memset(work->target, 0, sizeof(work->target));
    int nSize = bitsrev >> 24;
    uint32_t nWord = bitsrev & 0x007fffff;
    if (nSize <= 3) {
        nWord >>= 8 * (3 - nSize);
        work->target[0] = nWord;
    }
        ...
        memset(a, 0, sizeof(a));
        a[0] = nWord;
        unsigned int shift = 8 * (nSize - 3);
        int k = shift / 32;
        shift = shift % 32;
        for (int i = 0; i < 8; i++) {
            if (i + k + 1 < 8 && shift != 0)
                work->target[i + k + 1] |= (a[i] >> (32 - shift));
            if (i + k < 8)
                work->target[i + k] |= (a[i] << shift);
        }
    }

    /* generate merkle root */
    unsigned char merkle_root[32];
    sha256d(merkle_root, cbtx, cbtx_size);
    tmp = json_object_get(val, "merkleProof");
    if (tmp && json_is_array(tmp)) {
        int n = json_array_size(tmp);
        for (int i = 0; i < n; i++) {
            const char* branch_hex = json_string_value(json_array_get(tmp, i));
            int branch_len = strlen(branch_hex) / 2;
            unsigned char branch[branch_len];
            if(!hex2bin(branch, branch_hex, branch_len)) {
                applog(LOG_ERR, "JSON GMC invalid merkle proof");
                goto out;
            }
            memrev(branch, branch_len);

            unsigned char next[sizeof(merkle_root) + sizeof(branch)];
            memcpy(next, merkle_root, sizeof(merkle_root));
            memcpy(next + sizeof(merkle_root), branch, sizeof(branch));
            sha256d(merkle_root, next, sizeof(next));
        }
    }
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
    ...
    if (have_stratum) {
        ...
        char submit[256 + strlen(work->workid) + strlen(coinbasestr)];
        sprintf(submit,
            "{\"method\": \"submitminingsolution\", \"params\": [ {\"id\":\"%s\", \"nonce\":%u, \"coinbase\":\"%s\"} ], \"id\":1}\r\n",
            work->workid, nonce, coinbasestr);
        free(coinbasestr);

        val = json_rpc_call(curl, rpc_url, rpc_userpass, submit, NULL, 0);
        if (unlikely(!val)) {
        ...
        if (json_is_boolean(res)) {
            int sumres = json_is_true(res);
            if(sumres) {
                share_result(1, NULL);
            }
            else {
                res = json_object_get(val, "error");
                share_result(0, json_string_value(res));
            }
        }
        else {
            share_result(json_is_null(res), NULL);
        }
        json_decref(val);
    }
    else if (work->txs) {
